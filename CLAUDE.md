...
- Cron sweep auto-trigger + `CRON_SECRET` auth for `/api/archive/sweep` (deferred until deployment) — the sweep route and its manual Profile button are built; only the scheduled auto-run and bearer-token authentication remain.
- Tab 2 weekly plan.
