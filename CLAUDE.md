...
- A full hub with 7 tabs: Projects, AI Digest, Calendar, Clients, Areas, Resources, Calls
...
- `constants/project-views.ts` — **CREATED (2026-06-07).** The four named list-views for Tab 1: `PROJECT_VIEWS` (`open` = Active/In Progress/Later, `backlog` = Backlog, `onhold` = On Hold/Waiting, `done` = Done) + `ViewKey` type, `DEFAULT_VIEW_KEY` (`open`), and `PROJECT_VIEW_STATUSES` (the deduped union of every status any view references — consumed by `listProjectsForViews()`). Intentionally typed `readonly string[]` because it references Notion Status values outside the narrow `Status` union in `constants/priorities.ts`; that union stays limited to Active/On Hold/Done (Kanban columns + drawer/Add-dialog selects).
...
The Areas DB uses a **versioned-page** convention (Brand-OS-style `Name (vN)` suffixes + an `Archived` checkbox property): a base area like `Marketing` can have several pages, of which the most-recently-created non-archived one is "live" and the rest are "outdated"/"archived". Two surfaces sit on top of this, both linked from the Tab 5 header (an **Areas Review** outline button next to the title in [AreasView.tsx](app/areas/_components/AreasView.tsx)). All server logic lives in [lib/notion-areas.ts](lib/notion-areas.ts) (a second Notion client instance — see Library files).

- **Manage surface** ([app/areas/manage/page.tsx](app/areas/manage/page.tsx) → [ManageView.tsx](app/areas/manage/_components/ManageView.tsx)). Client component fetches `GET /api/areas/manage` on mount, buckets pages by **base name** (` (vN)` stripped via a local `normalize()`) into `live` / `outdated` / `archived` (live = newest non-archived; outdated = remaining non-archived; archived = checkbox-flagged). Per group it shows the live version + a list of outdated versions each with an **Archive** button (POSTs `/api/areas/archive`), an **Archive all outdated** action, and "Open in Notion" links. Optimistic-ish with reload-after; sonner toasts. The page shell only gates on `NOTION_AREAS_DB_ID`.
- **Review wizard** ([app/areas/review/page.tsx](app/areas/review/page.tsx) → [ReviewWizard.tsx](app/areas/review/_components/ReviewWizard.tsx)). A step-per-area wizard. On mount it POSTs `/api/areas/review/diff` to get each live area's done/new/ongoing project sets + the per-area `questions`. For each area: shows the project deltas, renders the question form (text/textarea/status/date inputs — status reuses the `areas.status.*` labels), then **Create draft** POSTs `/api/areas/review/draft` (Sonnet drafts the new version's properties + Markdown body; rendered as a preview with `react-markdown`). **Approve & write** POSTs the server-computed payload to `/api/areas/version` (creates `Name (vN+1)`, archives the previous version, archives the Done projects). Skippable areas (no movement) can be skipped; per-area `answers` / `drafts` / `written` are tracked in component state. Areas with no movement sort last; the wizard ends on a "Review complete" screen. The shell gates on both `NOTION_AREAS_DB_ID` + `NOTION_PROJECTS_DB_ID`.
- Constants: `ROUTES.pages.{areasManage, areasReview}` + `ROUTES.api.areas.{manage, version, archive, reviewDiff, reviewDraft}` in [constants/routes.ts](constants/routes.ts); the question bank in [constants/areas-review.ts](constants/areas-review.ts); ~60 `areasManage.*` / `areasReview.*` / `areasReview.q.*` i18n keys in [constants/translations.ts](constants/translations.ts) (DE+EN) + `areas.review`.
